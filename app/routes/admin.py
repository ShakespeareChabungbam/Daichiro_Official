...
from app.models import Admin, Client, Appointment, StudentAssessment, SiteConfig, SkillNestAdmission, Employee
...
def require_login():
    allowed_routes = ['admin.login', 'admin.forgot_password', 'admin.reset_password']
...
def login():
    if request.method == 'POST':
        ...
        if emp and bcrypt.check_password_hash(emp.password_hash, password):
            session['employee_id'] = emp.id
            session['emp_name'] = emp.name
            session['emp_designation'] = emp.designation or ''
            session['emp_must_change'] = emp.must_change_password
...
def forgot_password():
    if request.method == 'POST':
        ...
        if not user:
            ...
            flash('This email address is not registered. Please check and try again.', 'error')
            return render_template('admin/forgot_password.html')
        salt = f'pw-reset-{role}'
        token = generate_reset_token(email, salt=salt)
        reset_url = url_for('admin.reset_password', token=token, _external=True)
        sent = send_reset_email(email, reset_url, name=name)
        if not sent:
            flash('Could not send email. Check MAIL_USERNAME / MAIL_PASSWORD in .env.', 'error')
            return render_template('admin/forgot_password.html')
        flash('Reset link sent! Check your inbox — it is valid for 30 minutes.', 'success')
...
def reset_password(token):
    # Try admin salt first, then employee salt
    email = None
    role = None
    for r, salt in [('admin', 'pw-reset-admin'), ('employee', 'pw-reset-employee')]:
        try:
            email = verify_reset_token(token, salt=salt)
            role = r
            break
        ...
        if user:
            ...
            if new_username and new_username != user.username:
                if role == 'admin':
                    ...
                    existing = Employee.query.filter_by(username=new_username).first()
                if existing:
            ...
            db.session.commit()
            flash('Credentials reset successfully! Please log in with your new details.', 'success')
            return redirect(url_for('admin.login'))
...
def admin_profile():
    ...
    if request.method == 'POST':
        ...
        if form_type == 'username':
            ...
            elif len(new_username) < 3 or len(new_username) > 40:
            ...
            confirm_pw = request.form.get('confirm_password', '')
            errors = []
            if not bcrypt.check_password_hash(admin.password_hash, current_pw):
                errors.append('Current password is incorrect.')
            if len(new_pw) < 8:                               errors.append('At least 8 characters required.')
            if not re.search(r'[A-Z]', new_pw):              errors.append('Must include an uppercase letter.')
            if not re.search(r'[a-z]', new_pw):              errors.append('Must include a lowercase letter.')
            if not re.search(r'\d', new_pw):                 errors.append('Must include a number.')
            if not re.search(r'[!@#$%^&*(),.?":{}|<>_\-]', new_pw): errors.append('Must include a special character.')
            if new_pw != confirm_pw:                          errors.append('Passwords do not match.')
            if errors:
        ...
        return redirect(url_for('admin.admin_profile'))
    return render_template('admin/admin_profile.html', admin=admin, active_page='profile')
...
def dashboard():
    ...
    gemini_api_key = SiteConfig.get('gemini_api_key')
    # Only show last 8 chars for display, rest masked
    gemini_key_display = None
    if gemini_api_key:
        gemini_key_display = '••••••••' + gemini_api_key[-8:] if len(gemini_api_key) > 8 else '••••••••'
    return render_template('admin/dashboard.html', appointments=appointments, clients=clients,
                           assessments=assessments, active_page='overview',
                           counselling_fee=counselling_fee,
                           gemini_api_key=gemini_api_key,
                           gemini_key_display=gemini_key_display)

@admin_bp.route('/set-fee', methods=['POST'])
def set_fee():
    ...
    if fee:
        try:
            ...
            if fee_val < 0:
                raise ValueError
            SiteConfig.set('counselling_fee', str(fee_val))
            flash(f'Counselling fee updated to ₹{fee_val}.', 'success')
        except (ValueError, TypeError):
            flash('Invalid fee amount. Please enter a valid number.', 'error')
    else:
        SiteConfig.set('counselling_fee', None)
        flash('Counselling fee cleared — it will not be shown on the booking page.', 'success')
    return redirect(url_for('admin.dashboard'))

@admin_bp.route('/verify-gemini-auth', methods=['POST'])
def verify_gemini_auth():
    """Re-confirm admin credentials before allowing Gemini settings changes."""
    username = request.form.get('username', '').strip()
    password = request.form.get('password', '').strip()
    admin = Admin.query.filter_by(username=username).first()
    if admin and bcrypt.check_password_hash(admin.password_hash, password):
        session['gemini_auth_ok'] = True
        return jsonify({'ok': True})
    return jsonify({'ok': False, 'error': 'Invalid username or password.'}), 401


@admin_bp.route('/set-gemini-key', methods=['POST'])
def set_gemini_key():
    if not session.get('gemini_auth_ok'):
        flash('Authentication required to change Gemini settings.', 'error')
        return redirect(url_for('admin.gemini_settings'))
    api_key = request.form.get('api_key', '').strip()
    if api_key:
        SiteConfig.set('gemini_api_key', api_key)
        flash('Gemini API key saved successfully. AI assessments are now active.', 'success')
    else:
        SiteConfig.set('gemini_api_key', None)
        flash('Gemini API key cleared. AI will use fallback mock responses.', 'success')
    session.pop('gemini_auth_ok', None)  # clear after use
    return redirect(url_for('admin.gemini_settings'))


@admin_bp.route('/set-gemini-model', methods=['POST'])
def set_gemini_model():
    if not session.get('gemini_auth_ok'):
        flash('Authentication required to change Gemini settings.', 'error')
        return redirect(url_for('admin.gemini_settings'))
    model = request.form.get('model', 'gemini-2.5-flash').strip()
    allowed = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-2.0-flash-lite', 'gemini-2.0-flash-001', 'gemini-2.5-pro']
    if model in allowed:
        SiteConfig.set('gemini_model', model)
        flash(f'Model updated to {model}.', 'success')
    session.pop('gemini_auth_ok', None)
    return redirect(url_for('admin.gemini_settings'))


@admin_bp.route('/gemini-settings')
def gemini_settings():
    gemini_api_key = SiteConfig.get('gemini_api_key')
    gemini_key_display = None
    if gemini_api_key:
        gemini_key_display = '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022' + gemini_api_key[-8:] if len(gemini_api_key) > 8 else '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022'
    gemini_model = SiteConfig.get('gemini_model') or 'gemini-2.5-flash'
    valid_models = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-2.0-flash-lite', 'gemini-2.0-flash-001', 'gemini-2.5-pro']
    if gemini_model not in valid_models:
        gemini_model = 'gemini-2.5-flash'
        SiteConfig.set('gemini_model', gemini_model)
    return render_template('admin/gemini_settings.html',
                           active_page='gemini',
                           gemini_api_key=gemini_api_key,
                           gemini_key_display=gemini_key_display,
...
def clients_page():
    clients = Client.query.order_by(Client.created_at.desc()).all()
    return render_template('admin/clients.html', clients=clients, active_page='clients')
...
@admin_bp.route('/appointments/booking-status')
def booking_status_page():
    ...
    return render_template('admin/booking_status.html',
                           active_page='appointments',
                           booking_closed=booking_closed,
                           booking_message=booking_message,
                           booking_reopen_date=booking_reopen_date)

@admin_bp.route('/appointments/pricing')
def pricing_page():
    counselling_fee = SiteConfig.get('counselling_fee')
    offer_price = SiteConfig.get('offer_price')
    package_price = SiteConfig.get('package_price')
    ...
    return render_template('admin/pricing_management.html',
                           active_page='appointments',
                           counselling_fee=counselling_fee,
                           offer_price=offer_price,
                           package_price=package_price,
...
@admin_bp.route('/appointments/history')
...
def delete_appointment(appt_id):
    ...
    return redirect(url_for('admin.appointment_history_page'))

@admin_bp.route('/appointments/<int:appt_id>/send-invoice', methods=['POST'])
...
@admin_bp.route('/send-manual-invoice', methods=['POST'])
...
@admin_bp.route('/set-pricing', methods=['POST'])
def set_pricing():
    ...
    return redirect(url_for('admin.pricing_page'))

@admin_bp.route('/set-booking-status', methods=['POST'])
def set_booking_status():
    ...
    return redirect(url_for('admin.booking_status_page'))

@admin_bp.route('/appointment/<int:appointment_id>/mark-paid', methods=['POST'])
...
def assessments_page():
    ...
    return render_template('admin/assessments.html', assessments=assessments, clients=clients,
...
@admin_bp.route('/assessments/ai-done')
def assessments_ai_done():
    ...
    return render_template('admin/assessments_group.html', records=records, active_clients=active_clients,
                           active_page='assessments', group_key='ai-done',
                           group_title='AI Analysis Complete', group_icon='🧠',
                           group_color='emerald', back_url=url_for('admin.assessments_page'))

@admin_bp.route('/assessments/pending')
def assessments_pending():
    ...
    return render_template('admin/assessments_group.html', records=records, active_clients=active_clients,
                           ...
                           group_color='indigo', back_url=url_for('admin.assessments_page'))

@admin_bp.route('/assessments/in-progress')
def assessments_in_progress():
    ...
    return render_template('admin/assessments_group.html', records=records, active_clients=active_clients,
                           ...
                           group_color='amber', back_url=url_for('admin.assessments_page'))

@admin_bp.route('/assessments/trash')
def assessments_trash():
    ...
    return render_template('admin/assessments_group.html', records=records, active_clients=active_clients,
                           ...
                           group_color='red', back_url=url_for('admin.assessments_page'))

@admin_bp.route('/assessment/<int:assessment_id>/trash', methods=['POST'])
...
@admin_bp.route('/assessment/<int:assessment_id>/restore', methods=['POST'])
...
@admin_bp.route('/assessment/<int:assessment_id>/delete_permanent', methods=['POST'])
...
@admin_bp.route('/assessments/empty_trash', methods=['POST'])
def empty_trash():
    try:
...
def create_client():
    ...
    return redirect(url_for('admin.assessments_page'))

@admin_bp.route('/assessment/<int:assessment_id>/pdf')
def assessment_pdf(assessment_id):
    ...
    return render_template('admin/assessment_pdf.html', a=assessment, career_vision=career_vision)


@admin_bp.route('/upload-assessment', methods=['GET', 'POST'])
def upload_assessment():
    if request.method == 'POST':
        files = request.files.getlist('photos')
        if not files or all(f.filename == '' for f in files):
            flash('Please select at least one photo.', 'error')
            return redirect(url_for('admin.upload_assessment'))
...
"""
        ...
        try:
            ...
            return render_template('admin/review_scanned.html',
                                   data=extracted,
    ...
    return render_template('admin/upload_assessment.html', active_page='assessments')


@admin_bp.route('/save-scanned-assessment', methods=['POST'])
...
@admin_bp.route('/assessment/<int:assessment_id>/reanalyze', methods=['POST'])
...
def skillnest_page():
    ...
    return render_template('admin/skillnest_admin.html',
...
@admin_bp.route('/skillnest/settings')
def skillnest_settings_page():
    ...
    return render_template('admin/skillnest_settings.html',
                           admission_fee=admission_fee,
...
@admin_bp.route('/skillnest/add-admission')
def skillnest_add_admission_page():
    """SkillNest add manual admission page."""
    return render_template('admin/skillnest_add.html',
                           active_page='skillnest')


@admin_bp.route('/skillnest/records')
def skillnest_records_page():
    ...
    seats_left = max(0, total_seats - seats_taken) if total_seats else None
    return render_template('admin/skillnest_records.html',
                           admissions=admissions,
                           admission_fee=admission_fee,
                           total_seats=total_seats,
                           seats_taken=seats_taken,
...
@admin_bp.route('/skillnest/set-settings', methods=['POST'])
def skillnest_set_settings():
    ...
    if fee:
        try:
            ...
            flash('Invalid fee amount. Please enter a valid number.', 'error')
            return redirect(url_for('admin.skillnest_settings_page'))
    else:
...
@admin_bp.route('/skillnest/add-manual', methods=['POST'])
def skillnest_add_manual():
    ...
    if not child_name or not parent_mobile:
        ...
        return redirect(url_for('admin.skillnest_add_admission_page'))
    ...
    return redirect(url_for('admin.skillnest_records_page'))


@admin_bp.route('/skillnest/<int:admission_id>/edit', methods=['GET', 'POST'])
def skillnest_edit(admission_id):
    ...
    return render_template('admin/skillnest_edit.html', a=a, active_page='skillnest')


@admin_bp.route('/skillnest/<int:admission_id>/approve', methods=['POST'])
...
@admin_bp.route('/skillnest/<int:admission_id>/reject', methods=['POST'])
...
@admin_bp.route('/skillnest/<int:admission_id>/mark-paid', methods=['POST'])
...
@admin_bp.route('/skillnest/<int:admission_id>/delete', methods=['POST'])
...
@admin_bp.route('/assessments/manual', methods=['GET', 'POST'])
def manual_assessment():
    ...
    return render_template('admin/manual_assessment.html', active_page='assessments')
...
# Maps a stable key → (actual filename, label, ratio hint, pages used)
PHOTO_SLOTS = {
    'director': {
        'filename': 'DirectorPhoto.jpg',
        'label': 'Director Photo',
        'ratio': '3 : 4  (Portrait)',
        'hint': 'Ideal: 900 × 1200 px — portrait of the director/founder',
        'pages': ['Homepage', 'About Page'],
    },
    'homepage_1': {
        'filename': '1.jpg',
        'label': 'Homepage Banner — Left',
        'ratio': '16 : 9  (Landscape)',
        'hint': 'Ideal: 1600 × 900 px — full-width section background',
        'pages': ['Homepage'],
    },
    'homepage_2': {
        'filename': '2.jpg',
        'label': 'Homepage Banner — Right',
        'ratio': '16 : 9  (Landscape)',
        'hint': 'Ideal: 1600 × 900 px — full-width section background',
        'pages': ['Homepage'],
    },
    'skillnest_hero': {
        'filename': 'SN1.jpg',
        'label': 'SkillNest Hero Photo',
        'ratio': '4 : 3  (Landscape)',
        'hint': 'Ideal: 1200 × 900 px — hero/feature image on SkillNest page',
        'pages': ['SkillNest Page'],
    },
}
...
def _allowed_photo(file):
    """Validate BOTH extension AND actual image content via Pillow.
    Rejects any file that isn't a real image regardless of its filename.
    """
    ...
    if ext not in ALLOWED_EXTENSIONS:
        return False
    # Read file header to verify real image using Pillow
    try:
        from PIL import Image
        import io
        file.stream.seek(0)
        img = Image.open(io.BytesIO(file.stream.read(2048)))
        img.verify()          # raises exception if not a valid image
        file.stream.seek(0)   # reset for later .save()
    except Exception:
        return False
    return True
...
def photos_page():
    ...
    return render_template('admin/photos.html', active_page='photos', slots=slots)
...
def photos_upload():
    ...
    img_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'static', 'img')
    # Use slot filename (pre-defined, safe) — ignore user-supplied filename entirely
...
def employees_page():
    ...
    return render_template('admin/employees.html', employees=employees, active_page='employees')
...
def create_employee():
    ...
    # Generate username from name: "John Doe" → "john.doe"
    ...
    chars = string.ascii_letters + string.digits + '!@#$%'
    plain_pw = ''.join(random.choice(chars) for _ in range(10))
    hashed_pw = bcrypt.generate_password_hash(plain_pw).decode('utf-8')

    # Granular permissions from checkboxes
    permissions = {
        'appt_booking_status':      bool(request.form.get('perm_appt_booking_status')),
        'appt_pricing':             bool(request.form.get('perm_appt_pricing')),
        'appt_history':             bool(request.form.get('perm_appt_history')),
        'skillnest_admissions':     bool(request.form.get('perm_skillnest_admissions')),
        'skillnest_settings':       bool(request.form.get('perm_skillnest_settings')),
        'assessments_view':         bool(request.form.get('perm_assessments_view')),
        'assessments_create_client':bool(request.form.get('perm_assessments_create_client')),
        'gallery_view':             bool(request.form.get('perm_gallery_view')),
        'contact_social':           bool(request.form.get('perm_contact_social')),
    }
    ...
    try:
        db.session.add(emp)
        db.session.commit()
        flash(f'Employee "{name}" created! Username: {username} | Password: {plain_pw}', 'success')
    except Exception as e:
        db.session.rollback()
...
@admin_bp.route('/employees/<int:emp_id>/reset-password', methods=['POST'])
def reset_employee_password(emp_id):
    emp = Employee.query.get_or_404(emp_id)
    chars = string.ascii_letters + string.digits + '!@#$%'
    plain_pw = ''.join(random.choice(chars) for _ in range(10))
    emp.password_hash = bcrypt.generate_password_hash(plain_pw).decode('utf-8')
    emp.temp_password_hash = plain_pw
    emp.must_change_password = True
    db.session.commit()
    flash(f'Password reset for "{emp.name}". New password: {plain_pw}', 'success')
    return redirect(url_for('admin.employees_page'))


# ── Contact & Social Settings ────────────────────────────────────────────────

@admin_bp.route('/contact-settings')
def contact_settings_page():
    """Admin page to manage public contact info and social links."""
    return render_template('admin/contact_settings.html',
                           phone_primary=SiteConfig.get('contact_phone_primary'),
                           phone_secondary=SiteConfig.get('contact_phone_secondary'),
                           ...
                           social_linkedin=SiteConfig.get('social_linkedin'),
                           location_address=SiteConfig.get('location_address'),
                           location_map_url=SiteConfig.get('location_map_url'),
                           active_page='contact_settings')


@admin_bp.route('/contact-settings/save', methods=['POST'])
def contact_settings_save():
    """Save contact info and social links to SiteConfig."""
    fields = {
    ...
    return redirect(url_for('admin.contact_settings_page'))
